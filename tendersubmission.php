...
if (isset($_POST['regsucess'])) {
	...
	$concat_data = "SVT" . date("Ymd");
	date_default_timezone_set("Asia/Calcutta");
	$date = date("Y-m-d h:i:s a");

	$token = uniqid(date("Ymdhis"));
	$sql = "INSERT INTO tenderregistration(registration_id, postname, subpost, firstname, lastname, fathername, mothername, gender, dob, pannum, aadhaarno, mobno, email, vill, post, ps, block, dist, state, pincode, eduq, category, religion, bankname, accountno, ifsc, accounthname, img, concat_data, date, token_id, notify)
	...
	$sqll = "UPDATE tenderregistration SET registration_id = concat( concat_data, id ) where token_id = '$token'";
	if (mysqli_query($conn, $sql)) {
		if (mysqli_query($conn, $sqll)) {
			header("Location: tendersubmission.php?status=success&token=$token");
		} else {
			header("Location: tendersubmission.php?status=failed");
						...
						if ($_GET['status'] == "success") {
							echo '<div class="formsubmain">
                                    ...
                                    <div class="acceptterm">
                                        <center>
                                            <a href="tenderpdf.php?token=' . $token . '" target="_blank"><button><i class="fa fa-download" aria-hidden="true"></i> Download</button></a>
                                        </center>
                                    </div>
                                </div>';
